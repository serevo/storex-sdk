...
STOCK-MAN  (開発コードネーム: Storex [^1]) アプリ用モジュールを開発するための API のセットです。
...
- モジュールで使用可能な SDK のバージョンは、アプリで直接参照する SDK とメジャー番号が同じ且つマイナー番号が同じか低いものとしています。例えばアプリで v1.2.0 が参照されている場合、モジュールで使用可能なのは v1.0.0 以上 v1.3.0 未満です。これを遵守するようにします。
- 署名による厳密名について、厳密名を持つアセンブリ内のユーザー定義型は異なるバージョン間で別の型として扱われる為、また STOCK-MAN アプリで　SDK のバージョン リダイレクト (バインド リダイレクト) をしない為、この SDK では厳密名は付加しません。
